     ...
     1           .AND. name(ichar).EQ.refnam(iname)(ichar:ichar))
      ...
      ELSE IF ( iname.EQ.2 ) THEN
       IF (iniflg.EQ.1) THEN
        CALL ULMF_I__(xdata, xin, xout, xvar) ! Do not modify this line
       ELSE
        CALL ULMF_M__(xdata, xin, xout, xvar) ! Do not modify this line
       ENDIF
      CONTINUE !      -------------------------------------------
      ELSE IF ( iname.EQ.3 ) THEN
       IF (iniflg.EQ.1) THEN
        CALL icdll_init(xdata, xin, xout, xvar)
       ELSE
        CALL icdll_exec(xdata, xin, xout, xvar)
       ENDIF
      CONTINUE !      -------------------------------------------
